...
# graph algorithm for Mexico, to get the file of time_id_socre, eg: Mexico_id_score_two_20130911.json
# format: {time: {id: score}, ...}
# {"2013-09-10 15:00:00": {"8": 1.1925813666394531, "13": -0.2294157338705618, ...
def id_score_two():	
...
def daily_id_score_two():
	location_id = json.load( open("./location_id.json") ) 
	score_two = json.load(open("./absent_two_20130201_20140531_daily.json") )
	time_list = [k for k in score_two]
	...
	with open("./all_id_score_two.json","w") as output_file:
		json.dump(result, output_file)
...
# need to specify one day, eg: 2013-09-11
def	get_graph(t):
	G = nx.DiGraph()
	node_score = json.load( open("./all_id_score_two.json") ) #{"2013-04-19": {"2": -0.6430942968465125, "3": 1.2931034687028264,
...
#get_graph("2013-09-11")



# get the most absenteeism group
def minimal_weight_group(k,t):
	G = get_graph(t)
...
k=400
minimal_weight_group(k, "2014-02-27")
print k
print "end ", datetime.datetime.now()


#[875', 729', 1006', 345', 255', 1223', 835', 95', 665', 866', 720', 209', 1079', 49', 751', 129', 1221', 1047', 1044', 525']


def	get_final_lat_lon(t):
	group = minimal_weight_group(t)
	...
	with open("./minimal_graph_result_"+t, "w") as out:
		out.write(json.dumps(result))

#get_final_lat_lon("2014-02-27")




#####################################################################
# once get the first most absent group, as input, the top_group will return to get the second most absent group
def top_group(t):
	G = get_graph(t)	
	#group = minimal_weight_group(G)
	group = ['516', '842', '1128', '171', '591', '192', '562', '819', '719', '1176', '1243', '896', '399', '1291', '451', '260', '285', '373', '1170', '979']
